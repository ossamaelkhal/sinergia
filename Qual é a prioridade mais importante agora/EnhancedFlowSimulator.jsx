...
const FlowNode = ({ node, onDelete, onConnect, isConnecting, onNodeClick, connections, onStartConnection }) => {
  const nodeIcons = {
    trigger: Clock,
    action: Zap,
  ...
  const isSource = connections.some(conn => conn.from === node.id)
  const isTarget = connections.some(conn => conn.to === node.id)

  return (
    <div 
      className={`relative bg-white/10 border-2 rounded-lg p-4 cursor-pointer transition-all duration-300 hover:scale-105 ${
        isConnecting ? 'border-yellow-400 shadow-lg shadow-yellow-400/20 animate-pulse' : 
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${
          isConnected ? 'bg-green-500 animate-pulse' : 'bg-purple-500'
        }`}>
      ...
      <div className="absolute -left-2 top-1/2 transform -translate-y-1/2">
        <div 
          className={`w-4 h-4 rounded-full border-2 cursor-pointer transition-all duration-300 ${
            isTarget ? 'bg-blue-500 border-blue-400 animate-pulse' : 'bg-white/20 border-white/40 hover:bg-blue-400/60'
          }`}
          onClick={(e) => {
            e.stopPropagation()
      ...
      <div className="absolute -right-2 top-1/2 transform -translate-y-1/2">
        <div 
          className={`w-4 h-4 rounded-full border-2 cursor-pointer transition-all duration-300 ${
            isSource ? 'bg-green-500 border-green-400 animate-pulse' : 'bg-white/20 border-white/40 hover:bg-green-400/60'
          }`}
          onClick={(e) => {
            e.stopPropagation()
